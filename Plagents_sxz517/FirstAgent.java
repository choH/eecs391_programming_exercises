	...
	public Map middleStep(StateView newstate, HistoryView statehistory) {
        ...
        if (farmIds.size() < 1) {
	        if (currentGold >= 500 && currentWood >= 250) {
	                ...
	                actions.put(peasantID, Action.createCompoundBuild(peasantID, farmTemplateID, 20,10));
	        }
		}
		...
		if (barracksIds.size() < 1) {
	        if (currentGold >= 700 && currentWood >= 400) {
                ...
                actions.put(peasantID, Action.createCompoundBuild(peasantID, barrackTemplateID, 5,10));
	        }
		}
