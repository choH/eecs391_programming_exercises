...
public class MyCombatAgent extends Agent {
	...
	public Map<Integer, Action> initialStep(StateView newstate,
		...
		actions.put(footmanIDs.get(0), Action.createCompoundMove(footmanIDs.get(0),17,8));  // magic number discovered with groupmate.
			return actions;
	}
		...
		for (ActionResult feedback : statehistory.getCommandFeedback(playernum, currentStep-1).values()) {
			if (feedback.getFeedback() != ActionFeedback.INCOMPLETE) {
				for (int i=0;i<footmanIDs.size();i++) {
					actions.put(footmanIDs.get(0), Action.createCompoundMove(footmanIDs.get(0),11-i,8+i)); // magic number discovered with groupmate.
				}
			}
		}
